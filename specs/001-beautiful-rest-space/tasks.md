...
- [ ] T020 [P] 在 src/app/styles/tokens.css 定义轻、静、美的颜色、排版、间距和动效 token
...
- [ ] T047 [P] [US2] 在 src/test/e2e/us2-rest-space.spec.ts 覆盖 rest loading、rest active、fallback、completed、endedEarly 和返回工作 UI journey
- [ ] T048 [US2] 在 src/test/e2e/us2-rest-space-performance.spec.ts 验证 daily moment 或 polished fallback 95% 情况 2 秒内可见

### Implementation for User Story 2

- [ ] T049 [P] [US2] 在 src/features/rest-space/content/types.ts 实现 VisualMoment、AudioMoment、ContentCacheEntry 和 ProviderCandidate 类型
...
- [ ] T062 [US2] 在 src/app/styles/rest-space.css 实现全屏视觉、响应式控制项、防遮挡布局和 60fps 友好动效
...
- [ ] T069 [P] [US3] 在 src/features/rest-space/audio/types.ts 实现 AudioMoment、PlaybackState、VolumePreference 和 AudioUnavailableReason 类型
...
- [ ] T082 在 src/app/styles/tokens.css 和 src/app/styles/rest-space.css 完成 UX 一致性检查，覆盖 loading、empty、error、success、disabled、responsive 状态
...
Task: "T069 [P] [US3] 在 src/features/rest-space/audio/types.ts 实现 AudioMoment、PlaybackState、VolumePreference 和 AudioUnavailableReason 类型"
